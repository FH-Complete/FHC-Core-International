...
class Studiengangsleitung extends FHCAPI_Controller
{
	...
	public function __construct()
	{
		parent::__construct(array(
				'addStatus' => self::BERECHTIGUNG_KURZBZ .':rw',
	...
	public function load($studiengang)
	{
		...
		if (!in_array($studiengang, $stgBerechtigung))
			$this->terminateWithError($this->_ci->p->t('ui', 'keineBerechtigung'), self::ERROR_TYPE_GENERAL);

		$result = $this->_ci->InternatmassnahmezuordnungModel->getDataStudiengangsleitung(array($studiengang));
		$this->terminateWithSuccess(hasData($result) ? getData($result) : []);
	}

	public function loadBenotungen($studiengang, $studiensemester)
	{
		...
		$result = $this->_ci->InternatmassnahmezuordnungModel->getDataStudiengangsleitungBenotung($studiengang, $studiensemester);
		$this->terminateWithSuccess(hasData($result) ? getData($result) : []);
	}

	public function getLVs($studiengang, $studiensemester)
	{
		if (isEmptyString($studiengang) || isEmptyString($studiensemester))
			$this->terminateWithSuccess([]);
		...
		$this->terminateWithSuccess(hasData($lvs) ? getData($lvs) : []);
	}

	public function getOrgForms($studiengang, $studiensemester)
	{
		...
		$this->terminateWithSuccess(hasData($orgformen) ? getData($orgformen) : []);
	}
	...
	private function _setStatusMulti($data)
	{
		...
		$this->terminateWithSuccess(array('status_bezeichnung' => $status_bezeichnung, 'statusKurz' => $statusKurz));
	}
	...
	public function addStatus()
	{
		...
		if (isEmptyString($this->_ci->input->post('massnahme_id')))
			$this->terminateWithError('Error: Massnahme_id missing', self::ERROR_TYPE_GENERAL);

		if (isEmptyString($this->_ci->input->post('status')))
			$this->terminateWithError('Error: status missing', self::ERROR_TYPE_GENERAL);


		$massnahmeZuordnungPost = $this->_ci->input->post('massnahme_id');
		$statusPost = $this->_ci->input->post('status');
		$absagePost = !isEmptyString($this->_ci->input->post('absageGrund')) ? $this->_ci->input->post('absageGrund') : null;
		...
		if ($statusPost === 'declined' && !isEmptyString($absagePost))
		{
			...
			if (isError($updateStatus))
				$this->terminateWithError(getError($updateStatus), self::ERROR_TYPE_GENERAL);
		};
		$language = getUserLanguage() == 'German' ? 0 : 1;

		$this->sendMail($massnahmeZuordnung->massnahme_zuordnung_id);
		$this->terminateWithSuccess(array('massnahme' => $massnahmeZuordnung->massnahme_zuordnung_id, 'status' => $statusKurz, 'dms_id' => $massnahmeZuordnung->dms_id, 'status_bezeichnung' => $status->bezeichnung_mehrsprachig[$language], 'anmerkung_stgl' => $absagePost));
	}
	...
	public function setNote()
	{
		...
		if (!isset($postJson[0]) || !isset($postJson[0]->stg) || !isset($postJson[0]->lv) || !isset($postJson[0]->stsem))
			$this->terminateWithError("Error: Fehler beim Setzen der Note");
		...
		$this->terminateWithSuccess(array('count' => $count, 'students' => $students));
	}
	...
	public function getStudents()
	{
		...
		$this->terminateWithSuccess(is_null($filterStudent) ? [] : $filterStudent);
	}
	...
	private function _checkMassnahmenZuordnung($massnahmeZuordnungPost)
	{
		...
		if (isError($massnahmeZuordnung))
			$this->terminateWithError(getError($massnahmeZuordnung), self::ERROR_TYPE_GENERAL);
	...
	private function checkStatus($currStatus, $newStatus)
	{
		...
		if (isError($newStatus))
			$this->terminateWithError(getError($newStatus), self::ERROR_TYPE_GENERAL);
